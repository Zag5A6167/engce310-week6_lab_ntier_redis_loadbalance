...
Case A: จะสูงหน่อย  เพราะทุก Request ต้องวิ่งไปต่อคิวรอ PostgreSQL ดึงข้อมูลจาก Disk

Case B: จะต่ำลงมาก  เพราะ Redis เก็บข้อมูลไว้ใน RAM แล้ว ส่งกลับได้ทันทีโดยไม่ต้องถาม Database


Requests/sec:

Case A: ทำได้จำกัดตามแรงของ CPU ตัวเดียว

Case B: พุ่งสูงขึ้นหลายเท่า เพราะมี App 3 ตัว ช่วยกันรุมทำงาน และแต่ละตัวก็ทำงานเสร็จเร็วขึ้นเพราะมี Cache
